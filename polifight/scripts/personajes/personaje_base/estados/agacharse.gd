...
func on_input(_event)-> void:
	...
	elif not Input.is_action_pressed(derecha) and not Input.is_action_pressed(izquierda) and not Input.is_action_pressed(agacharse) and nodo_controlado.is_on_floor():
